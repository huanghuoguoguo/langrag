...
def assert_search_results_valid(results, min_results=0, max_results=float("inf")):
    ...
    for i, result in enumerate(results):
        assert isinstance(result, SearchResult), f"Result {i} is not a SearchResult"
        assert isinstance(result.chunk, Chunk), f"Result {i}.chunk is not a Chunk"
        assert 0 <= result.score <= 1.0, f"Result {i} score {result.score} not in [0, 1]"
...
def assert_chunks_have_embeddings(chunks):
    ...
    for i, chunk in enumerate(chunks):
        assert chunk.embedding is not None, f"Chunk {i} (id={chunk.id}) missing embedding"
