...
class ProviderFactory:
    ...
    STRATEGY_PRIORITY = [
        # (条件检查函数, Provider类, 描述)
        (lambda caps: caps.supports_hybrid, 
        ...
        (lambda caps: caps.supports_vector, 
         VectorSearchProvider, 
         "vector-only search"),
        
        (lambda caps: caps.supports_fulltext, 
         FullTextSearchProvider, 
         "fulltext-only search"),
    ]
    ...
    def create_for_store(
        ...
        # 没有匹配的策略
        raise ValueError(
            f"VectorStore '{store_name}' doesn't support any search mode. "
            f"Capabilities: {caps}"
        )
    ...
    def create_for_role(
        ...
        if role == Role.FULLTEXT_ONLY:
            if not caps.supports_fulltext:
                logger.warning(f"FULLTEXT_ONLY store '{store_name}' doesn't support fulltext search")
                return None
