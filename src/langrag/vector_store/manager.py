...
from ..config.models import StorageRole, VectorStoreConfig
...
class VectorStoreManager:
    ...
    def create_datasource(
        ...
        self._store_configs[name] = VectorStoreConfig(
            type=store_type, params=params, role=role, enabled=enabled
        )
    ...
    def get_store(self, name: str) -> BaseVectorStore | None:
        """获取数据源实例
    ...
    def get_config(self, name: str) -> VectorStoreConfig | None:
        """获取数据源配置
    ...
    def _make_cache_key(self, store_type: str, params: dict[str, Any]) -> tuple:
        ...
        try:
            ...
            frozen_params = frozenset((k, str(v)) for k, v in params.items())
    ...
    def get_reused_instances(self) -> dict[str, list[str]]:
        ...
        # 只返回被多个数据源共享的实例
        return {f"instance-{iid}": names for iid, names in reuse_map.items() if len(names) > 1}
