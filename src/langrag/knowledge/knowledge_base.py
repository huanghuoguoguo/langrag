...
class KnowledgeBase:
    ...
    def __init__(
        ...
        self.retrieval_config = retrieval_config or {"top_k": 5, "fusion_strategy": "rrf"}
    ...
    def _get_vector_stores(self) -> list[tuple[BaseVectorStore, StorageRole]]:
        ...
        for ds_name, role in self.datasource_refs:
            ...
            if store is None:
                logger.warning(f"Datasource '{ds_name}' not found in KB '{self.kb_id}', skipping")
                continue
        ...
        if not stores:
            raise ValueError(f"No valid datasources available for KB '{self.kb_id}'")
    ...
    def indexing_pipeline(self) -> IndexingPipeline:
        ...
        if self._indexing_pipeline is None:
            ...
            # 创建 parser
            parser = ComponentFactory.create_parser(ComponentConfig(**self.parser_config))

            # 创建 chunker
            chunker = ComponentFactory.create_chunker(ComponentConfig(**self.chunker_config))
            ...
            self._indexing_pipeline = IndexingPipeline(
                parser=parser, chunker=chunker, embedder=self.embedder, vector_stores=stores
            )
    ...
    def retriever(self):
        ...
        if self._retrieval_pipeline is None:  # 保留变量名以兼容
            ...
            if len(stores) == 1:
                ...
                # 多数据源：RRF 融合
                logger.info(
                    f"KB '{self.kb_id}': Using multi-store retrieval with {len(stores)} stores"
                )
    ...
    def index_file(self, file_path: str | Path) -> int:
        ...
        logger.info(
            f"KB '{self.kb_id}': Successfully indexed {num_chunks} chunks from '{file_path}'"
        )
        return num_chunks
    ...
    def retrieve(self, query: str, top_k: int | None = None) -> list[SearchResult]:
        ...
        logger.info(f"KB '{self.kb_id}': Retrieving for query: {query[:50]}...")

        # 使用工具函数处理同步/异步转换
        results = run_async_in_sync_context(self.retrieve_async(query, top_k))

        logger.info(f"KB '{self.kb_id}': Retrieved {len(results)} results")
        return results
    ...
    async def retrieve_async(self, query: str, top_k: int | None = None) -> list[SearchResult]:
        ...
        logger.info(f"KB '{self.kb_id}': Async retrieving for query: {query[:50]}...")
            ...
            "datasources": [
                {
                    ...
                    if self.store_manager.get_config(name)
                    else "unknown",
                }
    ...
    def __repr__(self) -> str:
        return f"KnowledgeBase(kb_id='{self.kb_id}', datasources={len(self.datasource_refs)})"
