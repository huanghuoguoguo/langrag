...
from ..config.models import StorageRole, ComponentConfig
...
class KnowledgeBaseManager:
    ...
    def create_knowledge_base(
        ...
        datasource_roles: Optional[List[str]] = None,
        ...
        try:
            ...
            datasource_refs = [
                ...
                for name, role in zip(datasource_names, datasource_roles)
            ]
            ...
            logger.info(
                f"Created knowledge base '{kb_id}' with "
                f"{len(datasource_names)} datasource(s)"
            )
    ...
    def _get_or_create_embedder(self, config: Dict[str, Any]) -> BaseEmbedder:
        ...
        if cache_key not in self._embedders:
            ...
            self._embedders[cache_key] = ComponentFactory.create_embedder(
    ...
    def _get_or_create_reranker(self, config: Dict[str, Any]) -> BaseReranker:
        ...
        if cache_key not in self._rerankers:
            ...
            self._rerankers[cache_key] = ComponentFactory.create_reranker(
                ComponentConfig(**config)
            )
        else:
